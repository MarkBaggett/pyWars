...
class exercise(object):
    ...
    def new_acct(self, uname, password, reg_code):
        ...
        resp = self.__post_json(
            url, {"user": uname, "password": password, "reg_code": reg_code}
        )
    ...
    def score(self, show_all=False):
        ...
        for name, score_tuple in sorted(
            sb.items(), key=lambda x: (x[1][0], _time_elapsed(x[1][2])), reverse=True
        ):
            ...
            if not self.print_rich_text:
                print(
                    f"{position:0>3}-{name[:15]: ^15} Points:{score:0>3}   Scored:{date_hour}   Completed:{finished}"
                )
            position += 1
    ...
    def data(self, qnum, overwrite=None):
        ...
        if not "blob" in json_blob:
            if self.print_rich_text:
                the_console.print(
                    "[red on bright_yellow] {} ".format(
                        json_blob.get("text", "UNABLE TO RETREIVE DATA")
                    )
                )
            else:
                print(
                    "*** {} ***".format(
                        json_blob.get("text", "UNABLE TO RETRIEVE DATA")
                    )
                )
            return None
        ...
        if isinstance(data_var, bytes) and data_var.startswith(b"PK"):
            ...
            if tgt_path.exists() and overwrite == None:
                write_file = (
                    input(
                        'The path already exists. Do you want to over write the current directory,"yes" or "no"? '
                    ).lower()
                    == "yes"
                )
            if overwrite != None:
    ...
    def answer(self, answer, notanswer=None):
        ...
        if notanswer:
            if self.show_answer_warnings:
                the_console.print(
                    "[red] Note: In this version .answer() does not require the question number. Only the answer."
                )
                the_console.print(
                    f"I fixed it for you this time and submitted .answer({notanswer})"
                )
            answer = notanswer
        if not self.loggedin:
            return "Please login first"
    ...
    def password(self, current_password, new_password):
        ...
        resp = self.__post_json(
            url, {"currentpass": current_password, "newpass": new_password}
        )
